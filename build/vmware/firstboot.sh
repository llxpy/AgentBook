#!/bin/sh
# agentbook · VMware 首次启动一键装机脚本（在 Alpine live ISO 控制台里执行一次）
#
...
# 注册 agentbook 开机自启服务；④ 关机。重启后从磁盘启动，浏览器即可访问。
#
...
for dev in /dev/sr0 /dev/sr1 /dev/cdrom /dev/cdrom0 /dev/cdrom1; do
  if mount -t iso9660 -o ro "$dev" "$MNT" 2>/dev/null; then
    if [ -f "$MNT/FIRSTBOO.SH" ] || [ -f "$MNT/AGENTBOOK.TGZ" ]; then CFG="$MNT"; break; fi
...
if [ -z "$CFG" ]; then
  ...
  if mount -t vfat -o ro /dev/fd0 "$MNT" 2>/dev/null && [ -f "$MNT/AGENTBOOK.TGZ" ]; then CFG="$MNT"; fi
fi
# HTTP 兜底：用法  sh firstboot.sh http://<宿主IP>:8000
if [ -z "$CFG" ] && [ -n "$1" ]; then
  echo "[firstboot] 从 HTTP 拉取应用包: $1"
  mkdir -p /tmp/cfg
  if wget -q "$1/AGENTBOOK.TGZ" -O /tmp/cfg/AGENTBOOK.TGZ 2>/dev/null && [ -f /tmp/cfg/AGENTBOOK.TGZ ]; then
    CFG=/tmp/cfg
  else
...
for p in sda1 sda2 sda3 sda4; do
  if mount -t ext4 -o ro /dev/$p /mnt/probe 2>/dev/null; then
    if [ -f /mnt/probe/etc/os-release ]; then
      NEWROOT="$p"
      if [ -f /mnt/probe/opt/agentbook/server.py ]; then APP_DONE=1; fi
    fi
...
echo "[firstboot] ③ 拷贝应用 → /mnt/new/opt/agentbook"
mkdir -p /mnt/new/opt/agentbook
tar xzf "$CFG/AGENTBOOK.TGZ" -C /mnt/new/opt/agentbook
...
chroot /mnt/new /bin/sh -c "
  ...
  cp /opt/agentbook/service/agentbook /etc/init.d/agentbook
  chmod +x /etc/init.d/agentbook
  rc-update add agentbook default
  echo agentbook > /etc/hostname
  # 写入网卡配置：Alpine 安装后默认 /etc/network/interfaces 可能为空，
  # 导致 OpenRC networking 解析失败，agentbook 因 need net 起不来。
  mkdir -p /etc/network
...
echo "    2) 重新开机 → 从磁盘启动，自动拉起 AgentBook 服务"
echo "    3) 浏览器访问 http://<VM的IP>:8080/"
echo "    4) 首次登录密码见 VM 内 /var/log/agentbook.log"
echo "==================================================================="
sleep 5
poweroff
